...
from collections import Counter # Recherche de caractčre le plus fréquent
...
def taille_cle(length, nbRepetitions, nbSequences):
    '''détermine la taille de la clé par mesure de la distance entre des répétitions dans le texte. On cherche 'nbRepetitions' répétitions de 'length' caractčres nbSequences fois.'''
...
def find_cle(tailleCle):
    # Séparation du texte en n morceaux correspondants aux caractčres encodés par le mĂŞme indice de la clé
    T = []
    ...
    for i in range(tailleCle):
        occurences = {}
        for j in T[i]: #recherche du caractčre le plus fréquent
            if j in occurences:
                occurences[j]+=1
            else :
                occurences[j] = 1
...
ref = ord(" ") # caractčre supposé le plus utilisé (e, a ou l'espace en francais)
# Affichage des résultats
cle = find_cle(taille_cle(4, 3, 4)) # taille de la séquence, nombre de répétitions, nombre de séquences vérifiant ces critčres
